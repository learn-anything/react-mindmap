...
var MindMap = function (_Component) {
  ...
  _createClass(MindMap, [{
    ...
    value: function fetchMap() {
      ...
      (0, _utils.getJSON)(this.props.url, function (res) {
        res.nodes.forEach(function (node) {
          return _this2.state.nodes.push(node);
        });
        res.connections.forEach(function (conn) {
          return _this2.state.connections.push(conn);
        });
        ...
        _this2.setState({
          connections: _this2.state.connections,
    ...
    value: function viewBox() {
      ...
      var Xs = this.state.nodes.map(function (node) {
        return node.location.x;
      });
      var Ys = this.state.nodes.map(function (node) {
        return node.location.y;
      });

      var minX = Math.round(Math.min.apply(Math, _toConsumableArray(Xs)) - 150);
      var minY = Math.round(Math.min.apply(Math, _toConsumableArray(Ys)) - 150);
      var maxX = Math.round(Math.max.apply(Math, _toConsumableArray(Xs)) - minX) + 150;
      var maxY = Math.round(Math.max.apply(Math, _toConsumableArray(Ys)) - minY) + 150;
    ...
    value: function renderConnections() {
      ...
      return this.state.connections.map(function (conn) {
        ...
        return React.createElement('path', { className: 'mindmap-connection', d: d.join(' ') });
      });
    }
  }, {
    key: 'renderNodes',
    ...
    value: function render() {
      ...
      if (!this.props.url && !this.state.fetched && this.props.connections.length > 0 && this.props.nodes.length > 0) {
        this.setState({
      ...
      return React.createElement(
        'svg',
        { viewBox: this.viewBox(), className: 'mindmap-svg' },
        React.createElement(
          'g',
          null,
